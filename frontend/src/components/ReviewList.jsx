    ...
    <div>
      <div className="flex flex-col sm:flex-row gap-6 mb-6">
        <div className="sm:w-48 flex-shrink-0">
          <div className="text-center p-4 rounded-xl" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-base)' }}>
            <p className="text-3xl font-bold" style={{ color: 'var(--accent-text)' }}>{stats.avgRating?.toFixed(1) || '0.0'}</p>
            <div className="flex items-center justify-center gap-0.5 my-1">
              {[1, 2, 3, 4, 5].map(s => (
                <Star key={s} size={14} fill={s <= Math.round(stats.avgRating || 0) ? 'var(--accent-text)' : 'none'} style={{ color: 'var(--accent-text)' }} />
              ))}
            </div>
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{stats.total || 0} reviews</p>
          </div>
